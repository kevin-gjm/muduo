...
#include <muduo/net/poller/SelectPoller.h>
...
Timestamp SelectPoller::poll(int timeoutMs, ChannelList* activeChannels)
{
	...
	FD_ZERO(&efds_); 
	int maxfd = 0;
	PollFdList::iterator itor;
	for (itor = pollfds_.begin();itor!= pollfds_.end();itor++)
	{
		
		if ((itor->events)&(POLLIN | POLLPRI | POLLRDHUP))
		{
			FD_SET(itor->fd,&rfds_);
		}
		else if((itor->events)&(POLLOUT))
		{
